...
if __name__ == '__main__':
    ...
    # Only target tokens 
    ft_only_target = FeatureTransformation(only_target, average_aggregation, sum_last_four)
    ...
    # Save stats as json
    with open("benchmarks/results/only_target_stats.json", "w") as fp:
        json.dump(bertic_stats,fp, indent=8) 

    # Save clf reports
    with open('benchmarks/results/only_target_clf_report.pkl', 'wb') as handle:
    ...
    plt.savefig('benchmarks/results/only_target_loss.jpg')
    plt.show()
    
    fig2 = plt.figure("Figure 2")
    plt.plot(epochs, f1, label='Test F1')
    plt.legend(loc="upper left")
    plt.savefig('benchmarks/results/only_target_f1.jpg')
    plt.show()
