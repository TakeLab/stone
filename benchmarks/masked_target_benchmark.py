...
if __name__ == '__main__':
    ...
    data['text'] = masked_target
    data['ner']  = '[MASK]'

    X = data[['document_id', 'text', 'ner', 'target_order']]
    ...
    # Only target tokens 
    ft_only_target = FeatureTransformation(only_cls, average_aggregation, last_layer)
    ...
    # Init model
    bertic = Bertic_NN(ft_only_target, 768, 3)
    bertic.to(device)
    ...
    # Save stats as json
    with open("benchmarks/results/masked_only_cls__last_layer_stats.json", "w") as fp:
        json.dump(bertic_stats,fp, indent=8) 

    # Save clf reports
    with open('benchmarks/results/masked_only_cls_clf_last_layer_report.pkl', 'wb') as handle:
    ...
    plt.savefig('benchmarks/results/masked_only_cls_last_layer_loss.jpg')
    plt.show()
    
    fig2 = plt.figure("Figure 2")
    plt.plot(epochs, f1, label='Test F1')
    plt.legend(loc="upper left")
    plt.savefig('benchmarks/results/masked_only_cls_last_layer_f1.jpg')
    plt.show()
